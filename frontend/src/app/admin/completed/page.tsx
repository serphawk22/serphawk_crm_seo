...
import {
  ...
  Tag, TrendingUp, Clock, Award, Filter, X
} from "lucide-react";
...
export default function CompletedTasksPage() {
  ...
  const stats = [
    ...
    { label: "High / Urgent Done", value: highCount + urgentCount, icon: TrendingUp, color: "text-amber-500", bg: "bg-amber-500/10" },
    { label: "Assigned Done",      value: tasks.filter(t => t.assignee_name).length, icon: User, color: "text-violet-500", bg: "bg-violet-500/10" },
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 p-4 md:p-6 space-y-6">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl p-4 shadow-sm flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-sm text-slate-800 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          />
          {search && (
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="hidden md:grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-4 px-6 py-3 border-b border-slate-100 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50">
          {["Task", "Priority", "Assignee", "Client", "Completed On"].map(h => (
            <p key={h} className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500">{h}</p>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-slate-400 dark:text-zinc-500 mt-1">
              {search || priorityFilter !== "All" ? "Try adjusting your search or filters." : "Tasks marked as Done will appear here."}
            </p>
          </div>
        ) : (
          <AnimatePresence>
                ...
                >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      {task.description && (
                        <p className="text-xs text-slate-400 dark:text-zinc-500 mt-0.5 line-clamp-1">{task.description}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-slate-500 dark:text-zinc-400">{dateStr}</span>
                  </div>
                </motion.div>
              );
            })}
