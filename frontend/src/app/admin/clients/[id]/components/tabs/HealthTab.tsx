...
function MetricCard({ icon: Icon, label, value, sub, color }: {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -2 }}
      className="p-5 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm overflow-hidden relative"
    >
      ...
      <p className="text-xs font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5">{label}</p>
    ...
    <div className="flex items-end gap-1 h-24">
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          ...
          />
          <span className="text-[9px] text-slate-400 dark:text-slate-600 font-bold">{d.label}</span>
        </div>
      ))}
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm">
          <h4 className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-4">{language === 'es' ? 'Actividad Esta Semana' : 'Activity This Week'}</h4>
          <SimpleBarChart data={weekData} color="bg-indigo-400 dark:bg-indigo-500" />
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 text-center">{language === 'es' ? 'Eventos por día (últimos 7 días)' : 'Events per day (last 7 days)'}</p>
        </div>

        <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm">
          <h4 className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-4">{language === 'es' ? 'Desglose de Comunicación' : 'Communication Breakdown'}</h4>
          <SimpleBarChart data={typeData} color="bg-violet-400 dark:bg-violet-500" />
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 text-center">{language === 'es' ? 'Por tipo de canal' : 'By channel type'}</p>
        </div>
      </div>

      {/* NPS & Engagement */}
      <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm">
        <h4 className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-4">{language === 'es' ? 'Métricas de Interacción' : 'Engagement Metrics'}</h4>
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={label} className="text-center p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
      ...
      {/* Timeline summary */}
      <div className="p-5 rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-sm">
        <h4 className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">{language === 'es' ? 'Actividad Reciente' : 'Recent Activity Feed'}</h4>
        {timeline.slice(0, 5).length === 0 ? (
          <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-4">{language === 'es' ? 'No hay actividad reciente' : 'No recent activity'}</p>
        ) : (
          <div className="space-y-2">
            {timeline.slice(0, 5).map((ev: any, i: number) => (
              <div key={i} className="flex items-center gap-3 py-1.5 border-b border-slate-50 dark:border-slate-800 last:border-0">
                ...
                <p className="flex-1 text-xs text-slate-600 dark:text-slate-400 truncate">{ev.title}</p>
