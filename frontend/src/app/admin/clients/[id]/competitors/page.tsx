...
function RadarMapLoader({ target, competitors, radiusKm }: { target: any; competitors: any[]; radiusKm: number }) {
  ...
  React.useEffect(() => {
    ...
    tm.addListener("click", () => {
      iwRef.current.setContent(`<div style="background:#18181b;color:#f4f4f5;padding:12px;border-radius:8px;min-width:200px"><div style="font-size:10px;color:#818cf8;font-weight:800;text-transform:uppercase;margin-bottom:6px">${t("admin_client_competitors.target_business_upper")}</div><div style="font-size:15px;font-weight:800">${target.name}</div><div style="font-size:11px;color:#a1a1aa;margin-top:4px">${target.address || ""}</div></div>`);
    ...
    competitors.forEach((c: any) => {
      ...
      const labelMap: Record<string, string> = { red: t("admin_client_competitors.direct_competitor"), orange: t("admin_client_competitors.strong_competitor"), yellow: t("admin_client_competitors.moderate"), green: t("admin_client_competitors.weak_competitor") };
      cm.addListener("click", () => {
        iwRef.current.setContent(`<div style="background:#18181b;color:#f4f4f5;padding:12px;border-radius:8px;min-width:220px"><div style="background:${PIN_COLORS[c.pin_color]};color:white;font-size:9px;font-weight:800;text-transform:uppercase;padding:3px 8px;border-radius:4px;display:inline-block;margin-bottom:8px">${labelMap[c.pin_color] || t("admin_client_competitors.target_business")}</div><div style="font-size:14px;font-weight:800;margin-bottom:4px">${c.name}</div><div style="font-size:11px;color:#a1a1aa;margin-bottom:8px">${c.address || ""}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:8px"><div style="background:#09090b;border-radius:6px;padding:4px 8px"><div style="font-size:9px;color:#71717a">${t("admin_client_competitors.distance")}</div><div style="font-size:13px;font-weight:700">${c.distance_km}km</div></div><div style="background:#09090b;border-radius:6px;padding:4px 8px"><div style="font-size:9px;color:#71717a">${t("admin_client_competitors.market_score")}</div><div style="font-size:13px;font-weight:700">${c.market_size_score}/100</div></div></div></div>`);
