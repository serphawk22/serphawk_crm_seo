  ...
  useEffect(() => {
    ...
    const runFullRadarFlow = async (overrideQuery?: string) => {
      try {
        ...
        if (!clientData) {
          const clientRes = await fetch(`${API_BASE_URL}/leads/${id}`);
          if (!clientRes.ok) throw new Error("Failed to fetch lead");
          const clientResJson = await clientRes.json();
          const leadObj = clientResJson.client || clientResJson;
          setClient(leadObj);
          clientData = leadObj;

          if (!leadObj.company_name) throw new Error("Lead missing company_name");
        }

        // Step 1: Locate Target on Google Maps
        setLoadingStep('locating_target');
        const query = overrideQuery || clientData.company_name;
        const searchRes = await fetch(`${API_BASE_URL}/radar/search`, {
          ...
          body: JSON.stringify({ 
            ...
            website: clientData.website || clientData.websiteUrl
          }),
        });
        ...
        const doManual = async () => {
            try {
                const searchRes = await fetch(`${API_BASE_URL}/radar/search`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ query: manualQuery, location_hint: client.city || client.state }),
                });
                if (!searchRes.ok) {
                  const errData = await searchRes.json().catch(() => null);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 dark:text-zinc-400 font-medium text-sm mt-1">
            {t("admin_client_competitors.subtitle")} {client?.company_name || client?.companyName || t("admin_client_competitors.target_client")}
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-slate-500 dark:text-zinc-400 mb-6 max-w-md">
              {error && error !== "An error occurred." && error !== "Manual search failed" ? (
                ...
                <>{t("admin_client_competitors.not_found_desc")} <strong>{client?.company_name || client?.companyName}</strong> {t("admin_client_competitors.not_found_desc_2")}</>
              )}
            </p>
