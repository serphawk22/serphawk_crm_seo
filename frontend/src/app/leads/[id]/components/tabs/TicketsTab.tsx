...
export default function TicketsTab({ leadId }: TicketsTabProps) {
  ...
  const fetchTickets = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/leads/${leadId}/tickets`);
      if (res.ok) {
  ...
  const handleUpdateStatus = async (ticketId: number, status: string) => {
    try {
      const res = await fetch(`${API_BASE_URL}/leads/${leadId}/tickets/${ticketId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
