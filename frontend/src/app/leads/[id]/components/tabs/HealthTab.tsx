...
export default function HealthTab({ lead, activities, emails, timeline, serviceRequests }: HealthTabProps) {
  ...
  const revenue = lead?.customFields?.total_revenue || '—';
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl border border-slate-200 dark:border-zinc-700 dark:border-slate-700/60 bg-white dark:bg-zinc-900 dark:bg-slate-900 shadow-sm">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: language === 'es' ? 'Visitantes Totales' : 'Total Visitors',   value: lead?.customFields?.total_visitors   || '—' },
            { label: language === 'es' ? 'Tasa de Interacción' : 'Engagement Rate',  value: lead?.customFields?.engagement_rate  || '—' },
            { label: language === 'es' ? 'Tiempo Prom. en Sitio' : 'Avg. Time on Site',value: lead?.customFields?.avg_time_on_site || '—' },
          ].map(({ label, value }) => (
