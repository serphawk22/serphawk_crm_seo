...
function ConversationCard({ conv, leadId, onRefresh }: { conv: any; leadId: any; onRefresh: () => void }) {
  ...
  const submitReply = async () => {
    ...
    setSubmitting(true);
    await fetch(`${API_BASE_URL}/leads/${leadId}/conversations/${conv.id}/replies`, {
      method: 'POST',
...
export default function ConversationsTab({ leadId, conversations, employees, currentUser, onRefresh }: ConversationsTabProps) {
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((conv: any) => (
            <ConversationCard key={conv.id} conv={conv} leadId={leadId} onRefresh={onRefresh} />
          ))}
        </div>
      )}
    </div>
